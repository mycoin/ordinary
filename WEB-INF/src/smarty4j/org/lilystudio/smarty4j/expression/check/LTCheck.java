...
/**
 * 小于操作布尔表达式节点, 检测第一个表达式是否弱类型小于第二个表达式
 * 
 * @version 0.1.3, 2008/12/12
 * @author 欧阳先伟
 * @since Smarty 0.1
 */
public class LTCheck extends BinaryCheck {
   ...
   *          表达式2
   */
  public LTCheck(IExpression exp1, IExpression exp2) {
    super(exp1, exp2);
  }
  ...
  protected void checkDouble(MethodVisitor mw, boolean swap) {
    mw.visitInsn(DCMPL);
    Utilities.visitSCInsn(mw, swap ? IFGT : IFLT, trueLabel, falseLabel);
  }
